...
function MonthReport() {
  ...
  const year = paramYear ? parseInt(paramYear, 10) : now.getFullYear();
  const month = paramMonth ? parseInt(paramMonth, 10) : now.getMonth() + 1;
  ...
  const loadMonthData = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      const res = await reportApi.getMonthDetail(year, month);
      if (res.success && res.data) {
        setData(res.data);
      } else {
        setError(res.message || "Failed to load month report.");
      }
    } catch (err) {
      setError(err.message || "Failed to load month report.");
    } finally {
      setLoading(false);
    }
  }, [year, month]);
  ...
  const goToPreviousMonth = () => {
    ...
    navigate(`/reports/${prevYear}/${prevMonth}`);
  };
  ...
  const goToNextMonth = () => {
    ...
    navigate(`/reports/${nextYear}/${nextMonth}`);
  };

  // Navigate to current month
  const goToCurrentMonth = () => {
    navigate(`/reports/${now.getFullYear()}/${now.getMonth() + 1}`);
  };

  // Switch month/year from dropdowns
  const handleDropdownChange = (newYear, newMonth) => {
    navigate(`/reports/${newYear}/${newMonth}`);
  };
  ...
  // Category totals for budget bars
      ...
      <main className="flex-1 p-4 md:p-8 lg:p-10 overflow-y-auto max-w-7xl mx-auto w-full">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              <div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-orange-400">
                ...
                <span>{data?.label || `${MONTH_NAMES[month - 1]} ${year}`}</span>
              </div>
            ...
            <h1 className="text-2xl md:text-4xl font-extrabold text-orange-500 tracking-tight">
              {data?.label || `${MONTH_NAMES[month - 1]} ${year}`} Financial Report
            </h1>
        ...
        <div className="bg-gray-900/90 border border-gray-800 rounded-2xl p-4 md:p-5 mb-8 shadow-xl backdrop-blur-sm">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select
                value={month}
                onChange={(e) => handleDropdownChange(year, parseInt(e.target.value, 10))}
              ...
              <select
                value={year}
                onChange={(e) => handleDropdownChange(parseInt(e.target.value, 10), month)}
                className="bg-gray-800 border border-gray-700 hover:border-orange-500/50 text-white text-sm font-semibold rounded-xl px-3.5 py-2 focus:outline-none focus:border-orange-500 transition cursor-pointer"
              >
                {yearOptions.map((y) => (
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white px-3.5 py-2 rounded-xl text-xs md:text-sm font-medium transition cursor-pointer border border-gray-700/60"
              >
              ...
              <button
                ...
                className={`px-3.5 py-2 rounded-xl text-xs md:text-sm font-semibold transition cursor-pointer border ${
                  isCurrentMonth
        ...
        {loading && !data ? (
          <div className="py-24 flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-gray-400 text-sm">Loading statement for {month}/{year}...</p>
          </div>
        ) : (
          <>
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
              ...
              <DashboardCard
                ...
                subtitle={`${data?.monthName || MONTH_NAMES[month - 1]} total verified expenses (${expenses.length} transactions)`}
                icon={TrendingUp}
                ...
                <div>
                  ...
                  {data?.highestCategory ? (
                    <p className="text-sm text-gray-300 leading-relaxed">
                      Your primary expenditure this month was on{" "}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-lg">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="text-lg font-bold text-white">Yearly Spending Trend ({year})</h2>
                </div>
            ...
            {Object.keys(budgets).length > 0 && (
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 mb-8">
                <h2 className="text-base font-bold text-white mb-4">
                  Category Budgets for {data?.monthName}
                </h2>
            ...
            {/* Statement Line Items (Expenses Table) */}
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-xl mb-8">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    Detailed ledger transactions for {data?.label}
                  </p>
                </div>
              ...
              {displayedExpenses.length === 0 ? (
                <div className="bg-gray-950/40 border border-gray-800 rounded-xl p-10 text-center text-gray-500">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {expenses.length === 0
                      ? `No expenses were recorded for ${data?.label}.`
            ...
            {/* Quick Available Monthly Statements Switcher */}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                  {availableMonths.map((m) => {
                    ...
                    return (
                      <button
                        key={m.key}
                        onClick={() => handleDropdownChange(m.year, m.month)}
